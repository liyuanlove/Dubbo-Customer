...
function timeElapse(sDate){
    var sdate = new Date(sDate.replace(/-/g, "/"));
    var now = new Date();
    var times = now.getTime() - sdate.getTime();
    var day = parseInt(times / (1000 * 60 * 60 * 24));
    times %= (1000 * 60 * 60 * 24);
    var hour = parseInt(times / (1000 * 60 * 60));
    times %= (1000 * 60 * 60);
    var minute = parseInt(times / (1000 * 60));
    times %= (1000 * 60);
    var second = parseInt(times / (1000));
    // var t = new Date(c)
    // var now = new Date();
    // var days = now.getTime() - t.getTime();
    ...
    //     f = "0"+f
    // }
    var a='<span class="digit">'+day+'</span> days <span class="digit">'+hour+'</span> hours <span class="digit">'+minute+'</span> minutes <span class="digit">'+second+"</span> seconds";
    $("#elapseClock").html(a)}
